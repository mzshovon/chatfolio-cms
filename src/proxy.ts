...
// Same-origin `/api/*` -> the real backend, so the browser only ever talks
// to its own origin and CORS never applies (see README "Talking to the
// backend"). This used to live in next.config.ts's `rewrites()`, which Next
// resolves ONCE at build time and bakes into `.next/routes-manifest.json`
// — the standalone server never re-reads it, so BACKEND_API_URL had to be a
// Docker build-arg, and pointing at a different backend meant rebuilding
// the image.
//
...
//
// Covers the whole /api/* namespace, not just /api/v1/* — most calls are
// /v1 (the authenticated candidate/admin API), but the anonymous feedback
// endpoint lives under /api/public instead (see src/lib/api/http.ts's
// `prefix` option), so this strips just the leading /api rather than
// hardcoding /v1.
//
...
// next.config.ts's own `rewrites()` handles `/api/*` directly instead,
...
export function proxy(request: NextRequest) {
  ...
  if (!backendUrl) {
    console.error(
      "BACKEND_API_URL is not set — every /api/* request will fail. " +
  ...
  const target = new URL(`${backendUrl}${rest}${request.nextUrl.search}`);
