...
`.env.local`/`.env.test`/`.env.production` loading, but since both are just `KEY=value`
files, the same file works for both without needing a Compose-only copy. Omitting
`--env-file` makes Compose fall back to a plain `.env` in this directory, which won't
exist unless you create one.

Same shape as the chatfolio-landing container: multi-stage `Dockerfile` (deps → build →
runner) producing a `node:24-alpine` image running Next's `output: "standalone"` server
as a non-root user, `docker-compose.yml` with `${PORT:-3001}` (3001, not 3000, so it can
run alongside a landing container on the same host) and the same `json-file` logging
caps. `npm run build` runs `next build --webpack`, not the Turbopack default, for the
